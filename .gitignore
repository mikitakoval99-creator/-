...
<head>
    <meta charset="UTF-8">
    <title>3D-Симулятор: Паркур, Кликер, Редактор, Ферма, Блогер</title>
    <style>
        body { margin: 0; overflow: hidden; font-family: Arial, sans-serif; background: #333; }
        
        /* --- ГЛАВНОЕ МЕНЮ И СТАРТОВЫЙ ЭКРАН --- */
        #menu-container, #ui {
        ...
        /* --- ОБЩИЕ СТИЛИ ДЛЯ ВЕРНУТЬСЯ В МЕНЮ --- */
        .back-to-menu {
        ...
        /* --- UI ИГРОВЫХ 3D РЕЖИМОВ (ПАРКУР/ФЕРМА/БЛОГЕР) --- */
        #crosshair {
...
<div id="menu-container">
    <div id="menu-box">
        ...
        <button class="mode-button" id="start-parkour">Паркур 🏃‍♂️ (3D)</button>
        <button class="mode-button" id="start-clicker">Кликер 💰 (2D)</button>
        <button class="mode-button" id="start-editor">Редактор Персонажа 🎨</button>
        <button class="mode-button" id="start-farm">Ферма 🌾 (3D)</button>
        <button class="mode-button" id="start-blogger">Блогер 📹 (3D)</button>
    </div>
</div>

<div id="game-container" style="display:none;"> 
    <button id="admin-fly-button">ADMIN: Полет</button> 
    <div id="crosshair"></div>
    <div id="trophy-message">🏆 ПОЗДРАВЛЯЕМ! РЕЖИМ ПОЛЕТА АКТИВИРОВАН! 🚀</div>
    <div id="ui" style="display:none;"> 
        <h1 id="welcome-message"></h1>
        <p id="info-message"></p>
        <p>КЛИКНИ СЮДА ЧТОБЫ ИГРАТЬ</p>
        <hr>
        <p>Управление: WASD + ПРОБЕЛ (прыжок/вверх)</p>
    </div>
</div>

<div id="clicker-ui">
    <div class="back-to-menu">← МЕНЮ</div>
    <div id="click-count">Клики: 0</div>
    <button id="click-button">КЛИК</button>
    <button class="upgrade-button" id="upgrade-50" data-cost="50">УЛУЧШЕНИЕ (Цена: 50)</button>
    <button class="upgrade-button" id="upgrade-90" data-cost="90" disabled>МЕГА УЛУЧШЕНИЕ (Цена: 90)</button>
    <button class="upgrade-button" id="upgrade-400" data-cost="400" disabled>СУПЕР УЛУЧШЕНИЕ (Цена: 400)</button>
</div>

<div id="farm-ui">
    <div class="back-to-menu" id="back-to-menu-farm">← МЕНЮ</div>
    <div id="farm-hud">
        <div id="money-display">Деньги: $20</div>
        <div id="inventory-display">
            Инвентарь:
            <span id="inv-carrot">Морковь: 0</span>, 
            <span id="inv-watermelon">Арбуз: 0</span>
        </div>
        <button id="shop-button">Открыть Магазин/Продажа 🛒</button>
        <p style="margin-top: 15px; font-size: 14px;">Кликни на грядку, чтобы посадить семена из инвентаря.</p>
    </div>
    ...
    <div id="shop-modal">
        ...
        <h3 style="margin-top: 20px;">💰 Продажа</h3>
        <div id="sell-items"></div>
        <button onclick="document.getElementById('shop-modal').style.display='none'" style="margin-top: 20px; padding: 10px; width: 100%;">Закрыть</button>
    </div>
</div>

<div id="editor-ui" >
    <div class="back-to-menu" id="back-to-menu-editor">← МЕНЮ</div>
    <div id="editor-controls">
        <h2>Настройка Персонажа</h2>
        <div class="control-group">
            <label for="head-color">Цвет Головы:</label>
            <input type="color" id="head-color" value="#FFE0B2">
        </div>
        <div class="control-group">
            <label for="shirt-color">Цвет Рубашки (Туловище):</label>
            <input type="color" id="shirt-color" value="#007BFF">
        </div>
        <div class="control-group">
            <label for="pants-color">Цвет Штанов (Ноги):</label>
            <input type="color" id="pants-color" value="#333333">
        </div>
        <div class="control-group">
            <label>Аксессуары:</label>
            <input type="checkbox" id="hat-toggle" checked>
            <label for="hat-toggle">Капелюх (Шляпа)</label>
        </div>
        <div class="control-group">
            <label for="face-texture-url">URL для Лица (Изображение):</label>
            <input type="text" id="face-texture-url" placeholder="Вставьте ссылку на изображение 1:1" value="">
            <small>Рекомендуется использовать квадратное изображение 🖼️</small>
        </div>
    </div>
</div>

<div id="blogger-ui">
    <div class="back-to-menu" id="back-to-menu-blogger">← МЕНЮ</div>
    <div id="blogger-hud">
        <h2>Студия Блогера</h2>
        <div id="stats-display">
            <p>💰 Деньги: $0</p>
            <p>👀 Просмотры: 0</p>
            <p>✅ Подписчики: 0</p>
            <p>⏱️ Просмотры/сек: 0</p>
        </div>
        
        <button id="shoot-video-button">Снять Видео (x1)</button>
...
<script>
    if (typeof THREE === 'undefined') { alert("Ошибка загрузки Three.js!"); }
    ...
    let money = 20; 
    const farmInventory = { carrot: 0, watermelon: 0 };
    const shopItems = {
        carrot: { cost: 10, sell: 100, growthTime: 10, name: "Морковь", icon: "🥕" },
        watermelon: { cost: 100, sell: 2000, growthTime: 20, name: "Арбуз", icon: "🍉" },
    };
    const plots = [];
    ...
    let bloggerUpgrades = {
        mic: { cost: 50, effect: 1, type: 'click', name: "Новый Микрофон 🎤", purchased: false },
        lighting: { cost: 200, effect: 5, type: 'click', name: "Проф. Свет 💡", purchased: false },
        editor: { cost: 500, effect: 1, type: 'auto', name: "Нанять Монтажёра 🧑‍💻", purchased: false },
        camera: { cost: 1000, effect: 10, type: 'click', name: "4K Камера 🎥", purchased: false },
        marketing: { cost: 2000, effect: 5, type: 'auto', name: "Маркетинг-бот 🤖", purchased: false },
    };
    ...
    document.getElementById('start-blogger').addEventListener('click', startBloggerMode); 
    ...
    function showMainMenu() {
        ...
        // 1. Сброс и скрытие ВСЕХ игровых UI
        ...
        document.getElementById('blogger-ui').style.display = 'none'; 
        
        // 2. Отмена циклов анимации
        if (animateId) cancelAnimationFrame(animateId);
        if (editorAnimateId) cancelAnimationFrame(editorAnimateId);
        if (farmAnimateId) cancelAnimationFrame(farmAnimateId);
        if (bloggerAnimateId) cancelAnimationFrame(bloggerAnimateId); 
        animateId = editorAnimateId = farmAnimateId = bloggerAnimateId = null;

        // 3. Удаление Canvas (очистка 3D-сцен)
        [renderer, editorRenderer, farmRenderer, bloggerRenderer].forEach(r => { 
            if (r && r.domElement) {
        ...
        // Сброс ссылок
        renderer = editorRenderer = farmRenderer = bloggerRenderer = null; 
        scene = editorScene = farmScene = bloggerScene = null; 
        controls = farmControls = bloggerControls = null; 
        
        // 4. Сброс общих обработчиков событий (важно для чистой работы)
    ...
    function performRegistration() {
        ...
        if (playerName === null || playerName.trim() === '') { playerName = 'Игрок'; }
        let playerBirthYear = prompt("Введите ваш год рождения:", "2000");
        
        document.getElementById('welcome-message').innerText = `${playerName.toUpperCase()}, ДОБРО ПОЖАЛОВАТЬ!`;
        document.getElementById('info-message').innerText = `Год рождения: ${playerBirthYear}.`;
    }

    // --- 1. РЕЖИМ ПАРКУРА: Инициализация и Логика Движения ---
    ...
    function createLevel() { 
        platforms.length = 0; 
        // Создание стартовых, вращающихся и движущихся блоков
        ...
        createTexturedBlock(0, -2, 0, 20, 4, 20, groundMat); // Start
    ...
    // --- 2. РЕЖИМ КЛИКЕРА: Инициализация и Логика ---
    function updateClickerUI() {
        document.getElementById('click-count').innerText = `Клики: ${clicks} (x${clickValue})`;
        ...
        if (!upgrade50Purchased) {
            ...
            upgrade50Btn.innerText = `УЛУЧШЕНИЕ (Цена: 50). Клик x2`;
        } else {
            upgrade50Btn.disabled = true;
            upgrade50Btn.innerText = `УЛУЧШЕНИЕ (КУПЛЕНО)`;
        }

        if (!upgrade90Purchased) {
            upgrade90Btn.disabled = clicks < 90 || !upgrade50Purchased; 
            upgrade90Btn.innerText = `МЕГА УЛУЧШЕНИЕ (Цена: 90). Клик x5`;
        } else {
            upgrade90Btn.disabled = true;
            upgrade90Btn.innerText = `МЕГА УЛУЧШЕНИЕ (КУПЛЕНО)`;
        }

        if (!upgrade400Purchased) {
            upgrade400Btn.disabled = clicks < 400 || !upgrade90Purchased;
            upgrade400Btn.innerText = `СУПЕР УЛУЧШЕНИЕ (Цена: 400). Клик x10`;
        } else {
            upgrade400Btn.disabled = true;
            upgrade400Btn.innerText = `СУПЕР УЛУЧШЕНИЕ (КУПЛЕНО)`;
        }
    }
    ...
    // --- 3. РЕЖИМ РЕДАКТОРА: Инициализация и Логика ---
            ...
            textureLoader.load(
                ...
                function(texture) {
                    const materials = [
                        ...
                        new THREE.MeshLambertMaterial({ color: charParts.headMat.color }), 
                        new THREE.MeshLambertMaterial({ color: charParts.headMat.color }), 
                        new THREE.MeshLambertMaterial({ map: texture }), // 5-я сторона - вперед
                        new THREE.MeshLambertMaterial({ color: charParts.headMat.color }) 
                    ];
    ...
    function initEditorScene() {
        ...
        editorScene.add(new THREE.HemisphereLight(0xffffff, 0x444444, 1));
    ...
    function setupEditorControls() {
        document.getElementById('head-color').addEventListener('input', createCharacterPreview);
        document.getElementById('shirt-color').addEventListener('input', createCharacterPreview);
        document.getElementById('pants-color').addEventListener('input', createCharacterPreview);
        document.getElementById('hat-toggle').addEventListener('change', createCharacterPreview);
        document.getElementById('face-texture-url').addEventListener('change', createCharacterPreview);
    }
    ...
    // --- 4. РЕЖИМ ФЕРМЫ: Инициализация и Логика ---
    function initFarmScene() {
        ...
        farmScene.add(new THREE.AmbientLight(0x404040, 0.8));
        ...
        farmControls.addEventListener('lock', function() {
            ...
            document.getElementById('interaction-message').innerText = "Нажмите, чтобы взаимодействовать с грядками. WASD для движения.";
        });
    ...
    function createFarmPlots() {
        ...
        const rows = 3;
        const cols = 3;
        const startX = -((cols * plotSize + (cols - 1) * spacing) / 2) + plotSize / 2;
        const startZ = -((rows * plotSize + (rows - 1) * spacing) / 2) + plotSize / 2;

        const dirtTexture = textureLoader.load(TEXTURES.dirt);
        dirtTexture.wrapS = dirtTexture.wrapT = THREE.RepeatWrapping;
        const plotMat = new THREE.MeshStandardMaterial({ map: dirtTexture });

        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                const x = startX + c * (plotSize + spacing);
                const z = startZ + r * (plotSize + spacing);
                
                const plot = new THREE.Mesh(new THREE.BoxGeometry(plotSize, 0.1, plotSize), plotMat);
                plot.position.set(x, -0.05, z);
                plot.userData = { isPlot: true, planted: null, growthStart: null, growthState: 0 };
                farmScene.add(plot);
                plots.push(plot);
            }
        }
    ...
    function updateFarmHUD() {
        document.getElementById('money-display').innerText = `Деньги: $${money.toFixed(2)}`;
        document.getElementById('inv-carrot').innerText = `Морковь: ${farmInventory.carrot}`;
        document.getElementById('inv-watermelon').innerText = `Арбуз: ${farmInventory.watermelon}`;
        ...
        Object.keys(shopItems).forEach(key => {
            ...
            div.innerHTML = `
                <span>${item.icon} ${item.name} - ${item.cost}$</span>
                <button class="buy-button" onclick="buyFarmItem('${key}')" ${money < item.cost ? 'disabled' : ''}>КУПИТЬ</button>
            `;
            shopDiv.appendChild(div);
        });
        
        // Обновление продажи
        const sellDiv = document.getElementById('sell-items');
        sellDiv.innerHTML = '';
        Object.keys(shopItems).forEach(key => {
            const item = shopItems[key];
            const count = farmInventory[key];
            const div = document.createElement('div');
            div.className = 'item-row';
            div.innerHTML = `
                <span>${item.icon} ${item.name}: ${count} шт. (1 шт. = ${item.sell}$)</span>
                <button class="buy-button" onclick="sellFarmItem('${key}')" ${count < 1 ? 'disabled' : ''}>ПРОДАТЬ ВСЕ</button>
            `;
            sellDiv.appendChild(div);
        });
    }

    window.buyFarmItem = function(key) {
        const item = shopItems[key];
        if (money >= item.cost) {
            money -= item.cost;
            farmInventory[key] += 1;
            updateFarmHUD();
            showMessage(`Куплено ${item.icon} ${item.name}!`, 1000);
        }
    }
    
    window.sellFarmItem = function(key) {
        const item = shopItems[key];
        const count = farmInventory[key];
        if (count > 0) {
            const totalEarning = count * item.sell;
            money += totalEarning;
            farmInventory[key] = 0;
            updateFarmHUD();
            showMessage(`Продано ${count} ${item.icon} за ${totalEarning.toFixed(2)}$!`, 2000);
        }
    }
    ...
    function handlePlotInteraction() {
        ...
        if (intersects.length > 0) {
            ...
            if (distance > INTERACTION_RANGE) {
                 showMessage("Слишком далеко от грядки.", 1000);
                 return;
            }
            ...
            if (data.growthState === 0) { 
                if (farmInventory[selectedItemToPlant] > 0) {
                    ...
                    const seed = new THREE.Mesh(new THREE.SphereGeometry(0.1, 8, 8), new THREE.MeshBasicMaterial({ color: 0xAAAAAA }));
                    seed.position.set(plot.position.x, 0.05, plot.position.z);
                    farmScene.add(seed);
                    plot.userData.plantMesh = seed;

                    updateFarmHUD();
                    showMessage(`Посажено ${item.icon} ${item.name}. Время роста: ${item.growthTime} сек.`, 2000);
                } else {
                    showMessage(`Нужно купить семена в магазине!`, 2000);
                }
                ...
                updateFarmHUD();
                showMessage(`Собрано ${item.icon} ${item.name}! Готово к продаже.`, 2000);
                 ...
                 showMessage(`${item.icon} ${item.name} растет. Осталось ${remaining} сек.`, 1500);
            }
        }
    }
    
    // --- 5. РЕЖИМ БЛОГЕРА: Инициализация и Логика ---
    ...
    function createStudioScene() {
        ...
        bloggerCamera.position.set(0, 1.6, 5); 

        // Свет
        bloggerScene.add(new THREE.HemisphereLight(0xffffff, 0x444444, 0.8));
        ...
        // Стол и ПК (точка взаимодействия)
        const desk = new THREE.Mesh(new THREE.BoxGeometry(2, 0.8, 1), new THREE.MeshStandardMaterial({ color: 0x8B4513 }));
        desk.position.set(0, 0.4, -4.5);
        bloggerScene.add(desk);

        const pc = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.6, 0.1), new THREE.MeshStandardMaterial({ color: 0x000000 }));
        ...
        bloggerControls.addEventListener('lock', function() {
            document.getElementById('blogger-ui').querySelector('#interaction-message').style.display = 'block';
            document.getElementById('blogger-ui').querySelector('#interaction-message').innerText = "Нажмите 'E' (или кликните кнопку) для съемки. WASD для движения.";
        });
        bloggerControls.addEventListener('unlock', function() {
            document.getElementById('blogger-ui').querySelector('#interaction-message').style.display = 'none';
        });
    }
    ...
    function startBloggerMode() {
        ...
        // Сброс статистики
        moneyBlogger = 0; views = 0; subscribers = 0; viewsPerClick = 1; autoViewsPerSecond = 0;
    ...
    function updateBloggerHUD() {
        document.getElementById('stats-display').innerHTML = `
            <p>💰 Деньги: $${moneyBlogger.toFixed(2)}</p>
            <p>👀 Просмотры: ${Math.floor(views)}</p>
            <p>✅ Подписчики: ${subscribers}</p>
            <p>⏱️ Просмотры/сек: ${autoViewsPerSecond}</p>
        `;
        document.getElementById('shoot-video-button').innerText = `Снять Видео (x${viewsPerClick} просмотров)`;
        
        const upgradeSection = document.getElementById('upgrade-section');
        upgradeSection.innerHTML = '<h3>Улучшения</h3>';
        
        // Генерация кнопок улучшений
        Object.keys(bloggerUpgrades).forEach(key => {
            ...
            btn.id = `upgrade-${key}`;
            btn.innerText = `${upgrade.name} - $${upgrade.cost} (x${upgrade.effect} ${upgrade.type === 'click' ? 'клик' : 'сек'})`;
    ...
    window.shootVideo = function() {
        ...
        const newViews = baseViews * (1 + subscribers / 100); 
        views += newViews;
        
        subscribers += Math.floor(newViews / 100);
        
        const moneyEarned = newViews / 100; 
        moneyBlogger += moneyEarned;
        
        showMessageBlogger(`+${newViews.toFixed(1)} Просмотров, +$${moneyEarned.toFixed(2)}!`, 1000);
        updateBloggerHUD();
    }
    ...
    window.buyUpgrade = function(key) {
        ...
        if (moneyBlogger >= upgrade.cost && !upgrade.purchased) {
            ...
            if (upgrade.type === 'click') {
                ...
                showMessageBlogger(`${upgrade.name} куплено! Просмотры за клик: +${upgrade.effect}!`, 2000);
            } else if (upgrade.type === 'auto') {
                autoViewsPerSecond += upgrade.effect;
                showMessageBlogger(`${upgrade.name} куплено! Просмотры/сек: +${upgrade.effect}!`, 2000);
            }
    ...
    function onBloggerKeyDown(event) {
        ...
        switch (event.code) {
            ...
            case 'KeyE': shootVideo(); break; // Клавиша взаимодействия для съемки видео
        }
    }
    ...
    function showMessageBlogger(text, duration = 2000) {
        const msg = document.getElementById('blogger-ui').querySelector('#interaction-message');
        msg.innerText = text;
        msg.style.display = 'block';
        clearTimeout(msg.timer);
        msg.timer = setTimeout(() => {
            if (bloggerControls && bloggerControls.isLocked === true) {
                // Восстанавливаем подсказку, если курсор заблокирован
                msg.innerText = "WASD для движения. ESC для меню.";
            } else {
                 msg.style.display = 'none';
            }
        }, duration);
    ...
    function animate() {
        ...
        if (controls && controls.isLocked === true) {
            ...
            // ... (далее код движения и коллизий для Паркура)
    ...
    function animateEditor() {
        ...
        if (characterGroup) {
            characterGroup.rotation.y += 0.01; // Вращение персонажа
        }
        ...
        plots.forEach(plot => {
            ...
            if (data.growthState === 1 && data.growthStart) {
                ...
                if (elapsed >= item.growthTime) {
                    data.growthState = 2; // Готово к сбору
                    if (plot.userData.plantMesh) farmScene.remove(plot.userData.plantMesh);
    ...
    function bloggerAnimate(time) {
        ...
        if (autoViewsPerSecond > 0) {
            ...
            moneyBlogger += autoViewsEarned / 100; 
            subscribers += Math.floor(autoViewsEarned / 100); 
        ...
        if (bloggerControls && bloggerControls.isLocked === true) {
            ...
            bloggerCamera.position.y = 1.6; // Фиксированная высота

            const pc = bloggerScene.getObjectByProperty('userData', 'action');
            if (pc && bloggerCamera.position.distanceTo(pc.position) < 2.5) {
                document.getElementById('blogger-ui').querySelector('#interaction-message').innerText = "Нажмите 'E' (или кликните кнопку) для съемки!";
            } else {
                 document.getElementById('blogger-ui').querySelector('#interaction-message').innerText = "WASD для движения. ESC для меню.";
            }
        }
    ...
    function onWindowResize() { 
        [camera, editorCamera, farmCamera, bloggerCamera].forEach(cam => {
            if (cam) {
                cam.aspect = window.innerWidth / window.innerHeight;
                cam.updateProjectionMatrix();
            }
        });
        [renderer, editorRenderer, farmRenderer, bloggerRenderer].forEach(rend => {
            if (rend) rend.setSize(window.innerWidth, window.innerHeight);
        });
    }
    ...
    function checkTrophyCollision() { 
        ...
        if (
            ...
            document.getElementById('trophy-message').innerText = "🏆 ПОЗДРАВЛЯЕМ! РЕЖИМ ПОЛЕТА АКТИВИРОВАН! 🚀";
        }
    }
    ...
    function toggleAdminFly() { 
        ...
        if (isFlying) {
            message.innerText = "ADMIN MODE: ПОЛЕТ АКТИВИРОВАН 🚀";
            message.style.display = 'block';
        } else {
            message.innerText = "ADMIN MODE: ПОЛЕТ ВЫКЛЮЧЕН 🛑";
    ...
    function onFarmKeyDown(event) {
        ...
        switch (event.code) {
            ...
            case 'ArrowRight': case 'KeyD': moveRight = true; break;
            case 'Space': 
                if (farmCamera.position.y <= 1.5) { velocity.y += 10; }
                break;
        }
    }
